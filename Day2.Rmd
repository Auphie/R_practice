...
```{R}
# List apply，將欲求取的函式or func()套用到每個vector上
x = list(c(1,2,3,4), c(5,6,7,8))
lapply(x, sum)

# Simply apply，Lapply的簡化說明
sapply(x,sum)
...
class(sapply(grades, sum))  # numeric
str(cdc)                    # 資料細項說明
names(cdc)                  # 資料欄位
```
#轉換資料類型指令
as.factor()  -> cdc$exerany = as.factor(cdc$exerany)
as.integer()
as.Date(now) 
...
# 常用df資料彙整指令
```{R}
table(c(1,4,4,3))    # 將不同的值計算個數
table(cdc$gender,cdc$exerany)    # groupby & count
summary(cdc$height)         # min Max mean quartile
# 設定下界/上界/間距後，將資料maping至該分類級距內：
cut(cdc$height,seq(45,95,by=5)) 
# 利用table()，將結果group by sum(包含下界不包含上界)
table(cut(cdc$height,seq(45,95,by=5),right=F))
```
# 敘述統計 
# mean(平均數)、中位數(median)、眾數(mod)
# 全距(range)、四分位數(quantile)、四分位差(IQS)
# 共變異數(cov)、相關係數(cor)
```{R}
mean(cdc$weight, na.rm = T)         # na.rm指遺漏值
median(cdc$weight, na.rm =  T)
which.max(x)                        # 取得最大數的index
names(which.max(table(cdc$weight))) # 取眾數的法1
names(sort(table(cdc$weight), decreasing = T)[1])  #法2
range(a)[2] - range(a)[1]           # 上界 - 下界
quantile(a,0.25)                    # 第一四分位數
quantile(a,0.5)
quantile(a,0.75)
IQR(cdc$weight)                     # 四分位距
fivenum(cdc$weight)                 # 四分位的5等分
summary(cdc$weight)                 # 四分位的綜合報告
boxplot(cdc$weight,horizontal = T)  # 快速地畫出盒鬚圖
var(cdc$weight)                     # 變異數
sqrt(var(cdc$weight))               # 標準差 法1
sd(cdc$weight)                      # 標準差 法2
# 共變異數 = sum((Xi - Xavg) * (Yi - Yavg)) / 2
cov()  -> cov(x,y)  or  cov(mtcars) # 資料必為int
# 相關係數 = cov(x,y) / (sd(x) * sd(y)) ->去除單位差距  
cor()  -> cor(x,y)  or  cor(mtcars[1:3])
```
...
# 做一個list儲存iris，list[1]儲存title name, list[2]儲存整個DF
flower= list(title="iris dataset", data= iris)
class(flower)
class(flower$data)
# 可以試：flower[1]與flower[2]來確認本list的結構
# flower$data是一個DF，所以裡面可以再用[]來操作
